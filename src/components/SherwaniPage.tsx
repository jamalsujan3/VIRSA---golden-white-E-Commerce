...
const FABRICS = ['Brocade', 'Silk', 'Velvet', 'Jacquard'];
...
export default function SherwaniPage({ products, onNavigate, onAddToCart }: PageProps) {
  const { toggle: toggleWishlist, has: isWishlisted } = useWishlist();
  // Filters State (only showing sherwani category)
  ...
  // Only filter sherwani products
  const sherwaniProducts = useMemo(() => {
    return products.filter((p) => p.category === 'sherwani');
  }, [products]);

  // Filter & Sort Logic
  const filteredProducts = useMemo(() => {
    let result = [...sherwaniProducts];
    ...
    return result;
  }, [sherwaniProducts, selectedSizes, selectedColors, priceRange, selectedFabrics, inStockOnly, sortBy]);
  ...
  return (
    <div id="sherwani-page-root" className="w-full">
      <SEO
        title="Imperial Wedding Sherwanis & Groomswear | VIRSA"
        description="Experience the royal standard of groomwear. Handcrafted wedding Sherwanis featuring antique Zari embroidery, Banarasi silk, and imperial silhouettes tailored in Dhaka."
        canonical="/category/sherwani"
        ogType="website"
        ogImage="https://images.unsplash.com/photo-1617627143750-d86bc21e42bb?w=1200&q=85&fit=crop"
        ogImageAlt="VIRSA Imperial Sherwani & Wedding Groomswear"
        structuredData={getBreadcrumbListSchema([
          { name: 'Home', path: '/' },
          { name: 'Sherwani', path: '/category/sherwani' }
        ])}
      />
      ...
      <section className="relative h-[45vh] w-full overflow-hidden bg-black flex items-center justify-center">
        <div
          ...
          style={{
            backgroundImage: `linear-gradient(to bottom, rgba(18,18,18,0.2), rgba(18,18,18,0.95)), url('https://images.unsplash.com/photo-1617627143750-d86bc21e42bb?w=1600&q=80&fit=crop')`
          }}
        />
        <div className="relative z-10 text-center max-w-3xl px-4 mt-8">
          ...
          <p className="text-xs sm:text-sm text-gray-300 font-sans tracking-wide leading-relaxed max-w-xl mx-auto">
            Experience the royal standard of groomwear. Handcrafted masterpieces featuring heavy antique Zari wiring, presidential structures, and authentic Banarasi silk.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="flex items-center justify-between pb-6 border-b border-white/10 mb-8">
          ...
          <p className="text-xs font-sans tracking-widest text-gray-300 uppercase">
            Showing {filteredProducts.length} of {sherwaniProducts.length} masterpieces
          </p>
        </div>
          ...
          <div className="lg:col-span-3 space-y-12">
            {displayedProducts.length === 0 ? (
              <div className="py-20 text-center space-y-4">
                <p className="text-sm text-gray-300">No luxury Sherwanis match your current selection criteria.</p>
                <button
                  onClick={resetFilters}
