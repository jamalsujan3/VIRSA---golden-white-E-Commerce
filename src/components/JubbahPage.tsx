...
const FABRICS = ['Linen', 'Viscose', 'Cotton'];
...
export default function JubbahPage({ products, onNavigate, onAddToCart }: PageProps) {
  const { toggle: toggleWishlist, has: isWishlisted } = useWishlist();
  // Filters State (only showing jubbah category)
  ...
  // Only filter jubbah products
  const jubbahProducts = useMemo(() => {
    return products.filter((p) => p.category === 'jubbah');
  }, [products]);

  // Filter & Sort Logic
  const filteredProducts = useMemo(() => {
    let result = [...jubbahProducts];
    ...
    return result;
  }, [jubbahProducts, selectedSizes, selectedColors, priceRange, selectedFabrics, inStockOnly, sortBy]);
  ...
  return (
    <div id="jubbah-page-root" className="w-full">
      <SEO
        title="Serene Islamic Jubbahs & Robes | VIRSA"
        description="Embrace minimal, modern grace with VIRSA Islamic Jubbahs, Emirati cuts, and breathable desert linen thobes tailored with tranquil refinement."
        canonical="/category/jubbah"
        ogType="website"
        ogImage="https://images.unsplash.com/photo-1552374196-1ab2a1c593e8?w=1200&q=85&fit=crop"
        ogImageAlt="VIRSA Serene Islamic Jubbah Collection"
        structuredData={getBreadcrumbListSchema([
          { name: 'Home', path: '/' },
          { name: 'Jubbah', path: '/category/jubbah' }
        ])}
      />
      ...
      <section className="relative h-[45vh] w-full overflow-hidden bg-black flex items-center justify-center">
        <div
          ...
          style={{
            backgroundImage: `linear-gradient(to bottom, rgba(18,18,18,0.2), rgba(18,18,18,0.95)), url('https://images.unsplash.com/photo-1552374196-1ab2a1c593e8?w=1600&q=80&fit=crop')`
          }}
        />
        <div className="relative z-10 text-center max-w-3xl px-4 mt-8">
          ...
          <p className="text-xs sm:text-sm text-gray-300 font-sans tracking-wide leading-relaxed max-w-xl mx-auto">
            Embrace minimal, modern grace. Tailored flowing silhouettes and thobes made from organic desert linen, lightweight Turkish cotton, and breathable soft viscose blends.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="flex items-center justify-between pb-6 border-b border-white/10 mb-8">
          ...
          <p className="text-xs font-sans tracking-widest text-gray-300 uppercase">
            Showing {filteredProducts.length} of {jubbahProducts.length} masterpieces
          </p>
        </div>
          ...
          <div className="lg:col-span-3 space-y-12">
            {displayedProducts.length === 0 ? (
              <div className="py-20 text-center space-y-4">
                <p className="text-sm text-gray-300">No luxury Jubbahs match your current selection criteria.</p>
                <button
                  onClick={resetFilters}
