...
export default function PanjabiPage({ products, onNavigate, onAddToCart }: PageProps) {
  const { toggle: toggleWishlist, has: isWishlisted } = useWishlist();
  // Filters State (only showing panjabi category)
  ...
  // Only filter panjabi products
  const panjabiProducts = useMemo(() => {
    return products.filter((p) => p.category === 'panjabi');
  }, [products]);

  // Filter & Sort Logic
  const filteredProducts = useMemo(() => {
    let result = [...panjabiProducts];
    ...
    return result;
  }, [panjabiProducts, selectedSizes, selectedColors, priceRange, selectedFabrics, inStockOnly, sortBy]);
  ...
  return (
    <div id="panjabi-page-root" className="w-full">
      <SEO
        title="Luxury Panjabi Collection 2026 | VIRSA Heritage Menswear"
        description="Discover exquisite 100% Giza Egyptian cotton, Mulberry silk, and artisanal embroidered Panjabis handcrafted for prestigious occasions, weddings, and Eid celebrations."
        canonical="/category/panjabi"
        ogType="website"
        ogImage="https://images.unsplash.com/photo-1617137968427-85924c800a22?w=1200&q=85&fit=crop"
        ogImageAlt="VIRSA Luxury Panjabi Collection"
      ...
      <section className="relative h-[45vh] w-full overflow-hidden bg-black flex items-center justify-center">
        <div
          ...
          style={{
            backgroundImage: `linear-gradient(to bottom, rgba(18,18,18,0.2), rgba(18,18,18,0.95)), url('https://images.unsplash.com/photo-1617137968427-85924c800a22?w=1600&q=80&fit=crop')`
          }}
        />
        <div className="relative z-10 text-center max-w-3xl px-4 mt-8">
          ...
          <p className="text-xs sm:text-sm text-gray-300 font-sans tracking-wide leading-relaxed max-w-xl mx-auto">
            Discover exquisite tunics crafted from premium Egyptian Giza cotton, handloom Mulberry silk, and pure organic linen, meticulously hand-embroidered by master craftsmen.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="flex items-center justify-between pb-6 border-b border-white/10 mb-8">
          ...
          <p className="text-xs font-sans tracking-widest text-gray-300 uppercase">
            Showing {filteredProducts.length} of {panjabiProducts.length} masterworks
          </p>
        </div>
          ...
          <div className="lg:col-span-3 space-y-12">
            {displayedProducts.length === 0 ? (
              <div className="py-20 text-center space-y-4">
                <p className="text-sm text-gray-300">No luxury Panjabis match your current selection criteria.</p>
                <button
                  onClick={resetFilters}
