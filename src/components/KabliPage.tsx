...
const FABRICS = ['Linen', 'Viscose', 'Cotton', 'Silk'];
...
export default function KabliPage({ products, onNavigate, onAddToCart }: PageProps) {
  const { toggle: toggleWishlist, has: isWishlisted } = useWishlist();
  // Filters State (only showing kabli category)
  ...
  // Only filter kabli products
  const kabliProducts = useMemo(() => {
    return products.filter((p) => p.category === 'kabli');
  }, [products]);

  // Filter & Sort Logic
  const filteredProducts = useMemo(() => {
    let result = [...kabliProducts];
    ...
    return result;
  }, [kabliProducts, selectedSizes, selectedColors, priceRange, selectedFabrics, inStockOnly, sortBy]);
  ...
  return (
    <div id="kabli-page-root" className="w-full">
      <SEO
        title="Royal Kabli Suits & Sets | VIRSA Atelier"
        description="Explore bespoke Kabli sets featuring structured drape, contrast piping, and premium Egyptian cotton crafted for royal elegance and masculine grandeur."
        canonical="/category/kabli"
        ogType="website"
        ogImage="https://images.unsplash.com/photo-1552374196-1ab2a1c593e8?w=1200&q=85&fit=crop"
        ogImageAlt="VIRSA Royal Kabli Suits Collection"
        structuredData={getBreadcrumbListSchema([
          { name: 'Home', path: '/' },
          { name: 'Kabli', path: '/category/kabli' }
        ])}
      />
      ...
      <section className="relative h-[45vh] w-full overflow-hidden bg-black flex items-center justify-center">
        ...
        <div className="relative z-10 text-center max-w-3xl px-4 mt-8">
          ...
          <p className="text-xs sm:text-sm text-gray-300 font-sans tracking-wide leading-relaxed max-w-xl mx-auto">
            Experience traditional sophistication with a modern silhouette. Impeccably tailored Kabli sets crafted from premium Egyptian cotton, fine linen, and rich blended fabrics.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="flex items-center justify-between pb-6 border-b border-white/10 mb-8">
          ...
          <p className="text-xs font-sans tracking-widest text-gray-300 uppercase">
            Showing {filteredProducts.length} of {kabliProducts.length} masterpieces
          </p>
        </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-xs font-mono text-gray-400">
                  <span>৳2,000</span>
                  <span className="text-[#C9A84C] font-semibold">৳{priceRange.toLocaleString()}</span>
                </div>
              </div>
            </div>
